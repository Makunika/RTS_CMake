...
TextRender::TextRender(int width, int height, std::string font) {

    shader = ResourceManager::loadShader("text");
    shader->use();
    glm::mat4 projectionText = glm::ortho(0.0f, static_cast<float>(width), 0.0f, static_cast<float>(height));
    shader->setMatrix4("projection", projectionText);
    ...
    // Путь к файлу шрифта
    std::string font_name = Utils::getFontPathString(font);
    if (font_name.empty())
    {
        std::cout << "ERROR::FREETYPE: Failed to load font_name" << std::endl;
        throw runtime_error("ERROR::FREETYPE: Failed to load font_name");
    }
